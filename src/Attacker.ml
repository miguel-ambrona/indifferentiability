...
type command =
  ...
  | F_cmd      of string * (string * int) * (string option)     (*    output * (input * id)     *)
  | R_cmd      of (string list) * block_cipher * (string list) * (string option)  (*    outputs * cipher * inputs *)
...
let rec aux_inline_adversary real expressions equations = function
  | [] -> expressions, equations
  | cmd :: rest_cmds ->
     let aux = aux_inline_adversary in
     begin match cmd with
     | Sample_cmd name -> aux real (Map.add expressions ~key:name ~data:(Leaf name)) equations rest_cmds
     | F_cmd (name, (var, id), fname) -> aux real (Map.add expressions ~key:name ~data:(F(Map.find_exn expressions var, id, fname))) equations rest_cmds
     | R_cmd (names, cipher, vars, oname) ->
        let new_variables =
          let vars = L.map vars ~f:(fun v -> Map.find_exn expressions v) in
          if real then cipher.cipher vars
          else L.map (range 0 (L.length names)) ~f:(fun i -> Rand (i, vars, oname))
        in
        let expressions' =
        ...
        in
        aux real expressions' equations rest_cmds
     ...
     | XOR_cmd (name, vars) ->
        ...
        in
        aux real (Map.add expressions ~key:name ~data:(simplify_expr new_expression) ) equations rest_cmds
     | Check (var1, eq, var2) ->
